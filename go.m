%Now the layout is given in a different way.
%For instance, {{1}, {2,3},{3,4,5},{5,6},{7}}
%i.e., a set of subsets.
%Therefore, it should now allow for any geometry, long range interactions,
%etc.
%The Hamiltonian is given in a similar way as before, a set of pairs
%[support], Operator
function [res, En] = go(layout)
sx = [0 1; 1 0]; sy = sqrt(-1)*[0 -1; 1 0]; sz = [1 0; 0 -1]; s0 = eye(2);
n=0;
for i = 1:numel(layout)
    for j = 1:size(layout{i},2)
        n = max(n,layout{i}(1,j));
    end
end
n=n+1;
%Now we attach the SdP variables to the layout elements
V={};
F= [];
energy = 0;
for i = 1:numel(layout)
    V={V{:},{layout{i},sdpvar(2^numel(layout{i}),2^numel(layout{i}))}};
...
%Let's try to build the cost function now.
flag = 0;
for i = 1:numel(H)
    %H{i}{1}
    found = 0;
    for j = 1:numel(V)
        if found == 0
            ...
            %pause;
                found = 1;
                %extH = extend_operator(H{i}{2},V{j}{1},H{i}{1});
                common = intersect(H{i}{1}, V{j}{1}); %indices are ordered here.
                mask = ismember(V{j}{1}, common).*1;
                order = intersect(V{j}{1},common, 'stable');
                for a = 0:(2^size(common,2)-1)
                    for b = 0:(2^size(common,2)-1)
                    tmp=0;
                        for c = 0:(2^(size(V{j}{1},2)-size(H{i}{1},2))-1) %Completes the index for the first state
                            %[1+indexmerge(a,c,maskLeft),1+indexmerge(b,c,maskLeft)]
                            %[a b c maskLeft]
                            tmp = tmp + V{j}{2}(1+indexmerge2(a,c,mask, order),1+indexmerge2(b,c,mask, order));
                        end
                    energy = energy + H{i}{2}(1+b,1+a)*tmp;
                    end
                end
            end
        end
